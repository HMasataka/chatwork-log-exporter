...
function populateForm(settings) {
  ...
  document.getElementById('exportCsv').checked = settings.exportCsv;
}
...
async function runExportScript(settings) {
  ...
  const EXPORT_CSV = settings.exportCsv;
  ...
  if (!token || !myid) {
    ...
    console.error('ACCESS_TOKEN:', typeof window.ACCESS_TOKEN, window.ACCESS_TOKEN);
    console.error('MYID:', typeof window.MYID, window.MYID);
    alert('エラー: Chatworkの認証情報が取得できませんでした。ページをリロードしてから再度お試しください。');
    return;
  }
  ...
  async function saveAs(filename, content) {
    ...
    if (typeof content === "string") {
      ...
      blob = new Blob([JSON.stringify(content, null, 2)], { type: "application/force-download" });
    }
  ...
  function convertMessagesToCSV(messages) {
    ...
    const headers = ['id', 'aid', 'aid_name', 'datetime', 'type', 'msg', 'tm', 'utm', 'index', 'reactions'];
    const csvRows = [headers.join(',')];
    for (const message of messages) {
      const values = [
        message.id || '',
        message.aid || '',
        message.aid_name ? `"${String(message.aid_name).replace(/"/g, '""')}"` : '',
        message.datetime ? `"${String(message.datetime).replace(/"/g, '""')}"` : '',
        message.type || '',
        message.msg ? `"${String(message.msg).replace(/"/g, '""').replace(/\n/g, '\\n')}"` : '',
        message.tm || '',
        message.utm || '',
        message.index || '',
        message.reactions ? `"${JSON.stringify(message.reactions).replace(/"/g, '""')}"` : ''
      ];
  ...
  async function init_load() {
    ...
    const resp = await do_fetch({ url: url, formData: { pdata: JSON.stringify({ _t: token }) } });
    return resp.json();
  }
  ...
  async function get_account_info(aids) {
    ...
    const resp = await do_fetch({ url: url, formData: { pdata: JSON.stringify({ aid: aids, _t: token }) } });
    return resp.json();
  }
  ...
  async function load_chat(rid) {
    ...
    const resp = await do_fetch({ url: url, formData: { pdata: JSON.stringify({ load_file_version: "2", _t: token }) } });
    return resp.json();
  }
  ...
  async function customize_messages(messages, aids) {
    ...
    for (const message of messages) {
      if (APPEND_DATE) message.datetime = yyyymmddhhmmss.format(new Date(message.tm * 1000));
      if (APPEND_USERNAME) message.aid_name = aids.result.account_dat[message.aid]?.name || "ユーザー名情報なし";
  ...
  async function downloadChatRoom(room_id) {
    ...
    const aids = messages.reduce((prev, message) => prev.add(message.aid), new Set());
    ...
    if (DOWNLOAD_ATTACHMENTS) {
      for (const file of load_chat_json.result?.file_list || []) {
        await saveAs(`${room_id}_${file.id}_${file.fn}`, await get_attachment_as_blob(file.id));
        await sleep(INTERVAL_TIME);
      }
    }
  }
  ...
  async function downloadChatRooms() {
    ...
    for (const [room_id, room_obj] of Object.entries(init_load_json.result.room_dat)) {
      const room_name = room_obj?.n || Object.entries(init_load_json.result?.contact_dat)?.filter(([k, v]) => v.rid == room_id)?.[0]?.[1]?.name || "ルーム名なし";
      if (EXCEPT_ROOM_IDS.includes(Number(room_id)) || (TARGET_ROOM_IDS.length !== 0 && !TARGET_ROOM_IDS.includes(Number(room_id)))) {
